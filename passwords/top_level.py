...
def Register_window(*event):
    ...
    def adder():
        if len(name.get()) and len(global_user_name.get()) and len(global_password.get()) and len(email.get()) > 0:
            try:
                tlb.add(name.get(),global_user_name.get(),global_password.get(),email.get())
                signin()
            except sq.IntegrityError:
                print("username or email alreday exists")
            
        else:
            print("given feilds cannot be empty")
