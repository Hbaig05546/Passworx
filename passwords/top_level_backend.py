...
def add(name, global_user_name, Global_Password, email,mobile):
    con = sq.connect("Global_Accounts.db")
    cur = con.cursor()   
    cur.execute("INSERT INTO Person VALUES(NULL,?,?,?,?,?)",(name, global_user_name, Global_Password, email,mobile))
    con.commit()
    con.close()
...
def update(account,name,userid,password,note,date,uniqueID):
    con = sq.connect("Global_Accounts.db")
    cur = con.cursor()
    cur.execute("UPDATE Account SET account=?,name_=?,userid=?,'password'=?,note=?,'date'=? WHERE uniqueID=?",(account,name,userid,password,note,date,uniqueID))
    con.commit()
    con.close()
...
def security_check(Global_user_name,Global_Password):
        con = sq.connect("Global_Accounts.db")
        cur = con.cursor()
        cur.execute("SELECT * FROM Person WHERE Global_user_name=? AND Global_Password=?",(Global_user_name,Global_Password)) 
